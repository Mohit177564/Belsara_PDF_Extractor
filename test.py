...
# === Extract Top-Right Bill Date (strict format) & Account Number ===
def extract_metadata(page_text: str) -> Dict[str, str]:
    bill_date = "0000"
    account_number = "0000"

    # Improved Bill Date regex (more robust to OCR noise)
...
def split_and_save(pdf_path: str, grouped: Dict[Tuple[str, str], List[int]], output_dir: str) -> List[str]:
    ...
    pages_text = extract_text_from_pdf(pdf_path)  # OCR once for all pages
    ...
    for (acc, _), pages in grouped.items():
        ...
        # Look for first valid bill date across the group
        for idx in pages:
